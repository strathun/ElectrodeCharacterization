...
%plots transparent traces and mean overlay
clear all;
...
%         semilogx(f,p)
        ...
        %gross, but removes 0 columns for means
        iii = iii + 1
        noiseArray(:,iii) = p*1e9;
        
        p1 = loglog(f,p*1e9,'color',colors(ii*spacer,:))
        p1.Color(4) = .15;
...
meanNoise = mean(noiseArray.');
h1 = loglog(f,meanNoise,'k','linewidth', 1);
