...
%     elecNum = str2num(currentNameStr(11:12))
    ...
    %added to build matrix with each channel impedance for averaging
    impArray( ii, : ) = sqrt(4*kT*Z(:,1,ii))*1e9 ; 
    
    totMag(ii, :) = Zmag(:,1,ii);
    totPhase(ii, :) = Phase(:,1,ii);
    
    himp = loglog(f(:,1,ii),sqrt(4*kT*Z(:,1,ii))*1e9,'--','color',colors(elecNum*spacer,:),'LineWidth',1);
    % Impedance
...
start = 10;
for jj = 1:L
    cumMeas(jj,:) = cumtrapz(frec(jj,start:end),prec(jj,start:end).^2);
    semilogx(fref(start:end),sqrt(cumMeas(jj,:))*1e6, 'linewidth', 1.5);
    hold on
end
