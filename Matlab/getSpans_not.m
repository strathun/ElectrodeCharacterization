function [ f_ret, PSD_ret ] = getSpans( saveFileNote, spans, centers, averages, sR, GPIBaddress )
    % close all;
    %--------------------------------------------------------------------------
    ...
    chop_bins=12; %chop off the first few PSD bins
...
%     averages = [32 32 64 64  64  64 64];
    ...
    for i=1:length(DCsupply.voltagesToSweep)
        ...
        if(save_all)
            ...
            C = clock;
            timestamp = sprintf('%dhr_%dmin_%.0fsec',C(4),C(5),C(6));
            run_name = sprintf('%s_%s_%s',datestamp,timestamp,saveFileNote);
            saveFileName = ['./pydevicecontrol/Noise_Test_Data/' run_name '.mat'];
            try
                save(saveFileName);
            catch MExc
                disp(MExc.message)
